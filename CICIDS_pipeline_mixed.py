...
class cicids_mixed_pipeline:

    def numericalize_feature_cicids(self, feature):
        # make all values np.float64
        
        feature = [
            np.float64(-1) if (x == "inf" or x == '' or x == "Infinity" or x == "NaN") else np.float64(
                x) for x in feature]
    ...
    def cicids_data_binary(self):
        filepath = os.path.join(
            working_directory(), 'bin-mixed-data', "train_data_features.csv")
        if (not os.path.isfile(filepath)):
            self.cicids_process_data_binary()

        filepath = os.path.join(
            working_directory(), 'bin-mixed-data', "train_data_features"
                                                   ".csv")

        normalized_train_data_features = np.loadtxt(filepath, delimiter=",")
        print('normalized_mixed_train_data_features finished!')
        filepath = os.path.join(
            working_directory(), 'bin-poisoned-data', "train_data_results.csv")
        normalized_train_data_results = np.loadtxt(filepath, delimiter=",")
        print('normalized_mixed_train_data_results finished!')
        filepath = os.path.join(
            working_directory(), 'bin-mixed-data', "test_data_features.csv")
        normalized_test_data_features = np.loadtxt(filepath, delimiter=",")
        print('normalized_mixed_test_data_features finished!')
        filepath = os.path.join(
            working_directory(), 'bin-mixed-data', "test_data_results.csv")
        normalized_test_data_results = np.loadtxt(filepath, delimiter=",")
        print('normalized_mixed_test_data_results finished!')
    ...
    def cicids_process_data_binary(self):
        ...
        train_data_9 = read_file_lines('cicids',
                                           'Friday-WorkingHours-Afternoon-DDos.pcap_ISCX.csv')
        train_data_9.pop(0)
        shuffle(train_data_9)

        train_data_10 = read_file_lines('cicids',
                                           'Friday-WorkingHours-Afternoon-PortScan.pcap_ISCX.csv')
        train_data_10.pop(0)
        shuffle(train_data_10)

        train_data_11 = read_file_lines('cicids',
                                           'Friday-WorkingHours-Morning.pcap_ISCX.csv')
        train_data_11.pop(0)
        shuffle(train_data_11)

        train_data_12 = read_file_lines('cicids',
                                           'Thursday-WorkingHours-Afternoon-Infilteration.pcap_ISCX.csv')
        train_data_12.pop(0)
        shuffle(train_data_12)

        train_data_13 = read_file_lines('cicids',
                                           'Thursday-WorkingHours-Morning-WebAttacks.pcap_ISCX.csv')
        train_data_13.pop(0)
        shuffle(train_data_13)

        train_data_14 = read_file_lines('cicids',
                                           'Tuesday-WorkingHours.pcap_ISCX.csv')
        train_data_14.pop(0)
        shuffle(train_data_14)

        train_data_15 = read_file_lines('cicids',
                                           'Wednesday-workingHours.pcap_ISCX.csv')
        train_data_15.pop(0)
        shuffle(train_data_15)

        train_data = train_data_1 + train_data_2 + train_data_3 + train_data_5 + \
                     train_data_9 + train_data_10 + train_data_11 + train_data_12 
        shuffle(train_data)
        ...
        mul_cicids = os.path.join(
            working_directory(), 'bin-mixed-data')
        ...
        filepath = os.path.join(
            working_directory(), 'bin-mixed-data', "train_data_results.csv")
