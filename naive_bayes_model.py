...
expected = y_mixed_test
predicted = model.predict(X_mixed_test)
...
conf_matrix = confusion_matrix(y_mixed_test, predicted)

# Plot the confusion matrix
plot_confusion_matrix(conf_matrix)

# Save the confusion matrix to a file
np.savetxt("mixed_data_classification_reports/nb_mixed_confusion_matrix.txt", conf_matrix, fmt='%d', delimiter=',')
...
results.to_csv("mixed_data_classification_reports/nb_mixed_predictions.csv", index=False)

print(classification_report(y_mixed_test, predicted, zero_division=0))

with open('mixed_data_classification_reports/nb_mixed_classification_report.txt', 'w') as f:
    f.write(classification_report(y_mixed_test, predicted, zero_division=0))

# Generate the classification report as a dictionary
report_dict = classification_report(y_mixed_test, predicted, output_dict=True)

# Convert the dictionary to a pandas DataFrame
report_df = pd.DataFrame(report_dict).transpose()

# Save the DataFrame to a CSV file
report_df.to_csv('mixed_data_classification_reports/nb_mixed_classification_report.csv')
